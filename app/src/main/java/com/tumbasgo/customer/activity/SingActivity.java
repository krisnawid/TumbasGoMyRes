...
public class SingActivity extends AppCompatActivity implements GetResult.MyListener {
        ...
        try {
            if (callNo.equalsIgnoreCase("1")) {
                ...
                if (response.getResult().equals("true")) {
                    Toast.makeText(SingActivity.this, "Nomor Telepon Sudah Terdaftar", Toast.LENGTH_LONG).show();
                } else {
                    User user = new User();
    ...
    public boolean validation() {
        if (edUsername.getText().toString().isEmpty()) {
            edUsername.setError("Anda Belum Memasukan Nama");
            return false;
        }
        if (edEmail.getText().toString().isEmpty()) {
            edEmail.setError("Masukan Email Dengan Benar");
            return false;
        }
